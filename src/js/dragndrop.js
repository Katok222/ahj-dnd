export function initDragAndDrop(state, rerender) {
  ...
  cards.forEach((cardEl) => {
    cardEl.addEventListener('mousedown', (e) => {
      ...
      const startColumn = state.columns.find(col => col.id === startColumnId);
      const startIndex = startColumn.cards.findIndex(c => c.id === cardId);
      ...
      function onMouseMove(event) {
        ...
        const container = targetColumnEl.querySelector('.column__cards');
      ...
      function onMouseUp() {
        ...
        const targetColumnEl = placeholder.closest('.column');
        const targetColumnId = targetColumnEl.dataset.columnId;
        const targetColumn = state.columns.find(col => col.id === targetColumnId);

        const placeholderIndex = [...placeholder.parentElement.children].indexOf(placeholder);

        // удаляем из старого места
        startColumn.cards.splice(startIndex, 1);

        let insertIndex = placeholderIndex;

        // фикс при перемещении вниз внутри той же колонки
        if (startColumnId === targetColumnId && startIndex < placeholderIndex) {
          insertIndex -= 1;
        }
